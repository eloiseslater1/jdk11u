...
[
  ...
  if test "x$ENABLE_AOT" = "xtrue"; then
    ...
    if test "x$OPENJDK_TARGET_CPU" = "xx86_64" || test "x$OPENJDK_TARGET_OS-$OPENJDK_TARGET_CPU" = "xlinux-aarch64" || test "x$OPENJDK_TARGET_OS-$OPENJDK_TARGET_CPU" = "xbsd-aarch64" ; then
      ...
      ENABLE_AOT="false"
      if test "x$enable_aot" = "xyes"; then
        AC_MSG_ERROR([AOT is currently only supported on x86_64, linux-aarch64 and bsd-aarch64. Remove --enable-aot.])
      fi
    fi
  fi
...
[
  ...
  if HOTSPOT_IS_JVM_FEATURE_DISABLED(jvmci); then
    ...
    if test "x$OPENJDK_TARGET_CPU" = "xx86_64" || \
       ...
       test "x$OPENJDK_TARGET_OS-$OPENJDK_TARGET_CPU" = "xbsd-aarch64" ; then
      AC_MSG_RESULT([yes])
