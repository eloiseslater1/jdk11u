...
#include <iostream> // offline testing - temporary hack
...
// Offline Testing
void
TemplateTable::testitem1() {
	CodeSection *testCode = __ code_section();
	address testStart = testCode->end();
	address testEnd = testCode->end();

	// now we add in the assembler section that we want to test
	__ addw(r7, r19, 340u);   // for example?
	__ addw(r7, r19, 340u);   // for example?
	__ addw(r7, r19, 340u);   // for example?
	__ addw(r7, r19, 340u);   // for example?
	__ addw(r7, r19, 340u);   // for example?
	...
	__ addw(r7, r19, 340u);   // for example?
	__ addw(r7, r19, 340u);   // for example?
	testEnd = testCode->end();
	std::cout << "If you have the dissassembly plugin installed then you will see the assemlber\n";
	std::cout << "Start address is  " << (void*)testStart << " and upper bound is " << (void*)testEnd << std::endl;
	std::cout << "One can see that there is some preamble generated by the interpreter before the start address\n";

	std::cout << "Beginnings of some code to print it out into a file/harness\n" << std::endl << std::endl;
	for(address machineCode = testStart; machineCode < testEnd; machineCode = machineCode + 4) {
		std::cout << (void*) machineCode << " " << (void*) *(unsigned int *)machineCode << std::endl;
	}
}
